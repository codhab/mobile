...
module Social
  class FormsController < ApplicationController
    ...
    def new
      @current_form = @form.form_values.where("store->> 'cadastre_id' = '?'", current_social.id).first rescue nil
    ...
    def create
      @form_value = @form.form_values.new
      @form_value.write_attributes
      @form_value.cadastre_id = current_social.id
    ...
    def show
      @form_value = @form.form_values.where("store->> 'cadastre_id' = '?'", current_social.id).first
    end
